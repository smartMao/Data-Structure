...
int main()
{
	// 实现: 取双链表 node1  与 node2 的交集，存放到 nodeIntersect
	...
	insert_data_into_double_link(node2, 8);
	insert_data_into_double_link(node2, 10); 

	DOUBLE_LIKE_NODE *nodeIntersect;
	nodeIntersect = (DOUBLE_LIKE_NODE *)malloc(sizeof(DOUBLE_LIKE_NODE));
	*nodeIntersect = double_link_node_intersect(*node1, *node2);

	printf("交集：\n");
	delete_head_double_link_node(nodeIntersect);
	print_double_link_node(nodeIntersect);
