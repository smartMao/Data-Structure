...
int switchNodeFlag = 1;

// 实现： 编写一个程序，删除某节点，规则是：交替使用 左子树中 最大的节点 和 右子树中 最小的节点 来代替 “被删除的节点”
...
int main()
{
	...
	Delete(5, t); // 先用 左子树中的 最大来代替
	Delete(4, t); // 再用 右子树中的 最小来代替
}
	...
	if (t == 0) {
		...
		if (elem < cursorSpace[t].element) {
			...
			if (cursorSpace[t].left && cursorSpace[t].right) {
				...
				// 交替使用 左子树中 最大的节点 和 右子树中 最小的节点 来代替 “被删除的节点”
				if (switchNodeFlag) {
					switchNodeFlag = 0;
